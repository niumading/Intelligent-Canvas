...
2. **首次使用**：右键点击 `mac-初始化环境.command` → 选择“打开”，安装依赖并构建前端
3. 右键点击 `mac-启动服务.command` → 选择 "打开" → 点击 "打开"（绕过安全警告）
4. **之后使用**：直接双击 `mac-启动服务.command` 即可
...
- **独立初始化环境**：首次运行 `mac-初始化环境.command` 安装所需依赖
...
| 文件 | 说明 |
|------|------|
| `mac-初始化环境.command` | 首次使用时安装依赖并构建前端 |
