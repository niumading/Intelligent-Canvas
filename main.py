...
def fetch_remote_update_notes(url: str, version: str = "", timeout: float = 5.0) -> Dict[str, Any]:
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "Company-Canvas"},
            timeout=timeout,
...
def check_update():
    """This project manages releases independently."""
    return {"current": current_app_version(), "github": {}, "modelscope": {}, "latest": {}, "update_notes": {}, "update_notes_sources": {}, "update_available": False, "reachable": True}
...
def github_bytes(url: str) -> bytes:
    resp = github_get(url, headers={"User-Agent": "Company-Canvas"}, timeout=60)
    return resp.content
...
def modelscope_update_file_list() -> List[str]:
    ...
    resp = github_get(MODELSCOPE_TREE_URL, headers={"User-Agent": "Company-Canvas"}, timeout=30)
    payload = json.loads(resp.content.decode("utf-8", errors="replace"))
    ...
    try:
        if os.name == "nt":
            ...
            script = (
                ...
                "  start \"Intelligent Canvas\" /D \"%APP_DIR%\" cmd /k call \"%LAUNCHER%\"\r\n"
                ") else (\r\n"
                "  echo [%date% %time%] launcher missing, fallback to python main.py >> \"%LOG_FILE%\"\r\n"
                "  if exist \"%APP_DIR%\\python\\python.exe\" (\r\n"
                "    start \"Intelligent Canvas\" /D \"%APP_DIR%\" cmd /k \"\"%APP_DIR%\\python\\python.exe\" main.py\"\r\n"
                "  ) else (\r\n"
                "    start \"Intelligent Canvas\" /D \"%APP_DIR%\" cmd /k python main.py\r\n"
                "  )\r\n"
                ")\r\n"
                "del \"%~f0\"\r\n"
            )
...
def update_from_github(req: UpdateRequest = UpdateRequest()):
    raise HTTPException(status_code=403, detail="此项目的在线更新已停用")
...
def rollback_update(req: RollbackRequest):
    raise HTTPException(status_code=403, detail="此项目的在线回滚已停用")
    if not req.name:
...
def fetch_remote_media_bytes(url: str, timeout: float = 30.0, max_bytes: int = 200 * 1024 * 1024):
    ...
    if parsed.scheme not in ("http", "https") or not parsed.netloc:
        return None
    with requests.get(text, stream=True, timeout=timeout, headers={"User-Agent": "Company-Canvas/1.0"}) as response:
        response.raise_for_status()
...
def download_output(request: Request, url: str, name: str = "", inline: bool = False):
    ...
    try:
        upstream_headers = {"User-Agent": "Company-Canvas/1.0"}
...
class WhiteModelExportRequest(BaseModel):
    data: str = ''
    name: str = '白模镜头.png'
    canvas_id: str = ''

@app.post('/api/white-model/send-to-canvas')
async def send_white_model_to_canvas(payload: WhiteModelExportRequest):
    raw = (payload.data or '').strip()
    if not raw.startswith('data:image/png;base64,'):
        raise HTTPException(status_code=400, detail='只支持 PNG 截图')
    try:
        encoded = raw.partition(',')[2]
        if len(encoded) > 28 * 1024 * 1024:
            raise ValueError('截图超过 20MB')
        content = base64.b64decode(encoded, validate=True)
        if not content or len(content) > 20 * 1024 * 1024:
            raise ValueError('截图为空或超过 20MB')
        with Image.open(BytesIO(content)) as picture:
            if picture.format != 'PNG':
                raise ValueError('截图格式不是 PNG')
            picture.verify()
    except Exception:
        raise HTTPException(status_code=400, detail='无效的 PNG 截图')
    canvas = load_canvas(payload.canvas_id) if payload.canvas_id else new_canvas('3D 白模镜头', '🧍', 'classic')
    filename = f'white_model_{uuid.uuid4().hex[:12]}.png'
    path = output_path_for(filename, 'input')
    with open(path, 'wb') as file:
        file.write(content)
    count = len(canvas.get('nodes') or [])
    node = {
        'id': f'img_{uuid.uuid4().hex[:12]}', 'type':'image',
        'x': 120 + (count % 5) * 36, 'y': 120 + (count % 5) * 36,
        'url': output_url_for(filename, 'input'), 'name': (payload.name or '白模镜头.png')[:100],
        'mediaKind':'image',
    }
